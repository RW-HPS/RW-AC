...
import net.rudp.RUDPServerSocket;
import net.rudp.c;
...
public class ServerAcceptRunnable implements Runnable {
    ...
    /* renamed from: a */
    public final boolean f568a = false;
    ...
    public ServerAcceptRunnable(GameNetEngine gameNetEngine) {
        this.netEngine = gameNetEngine;
    }

    public boolean a(InetAddress inetAddress, boolean z) {
        if (inetAddress == null) {
            GameEngine.m2e("isIpAllowed: inetAddress==null");
            return true;
        } else if (!b) {
            return true;
        } else {
            ...
            synchronized (this.l) {
                ...
                while (true) {
                    ...
                    if (inetAddress.equals(serverAcceptData.f570a)) {
                        serverAcceptData.b++;
                        int i = 30;
                        if (this.n > 100) {
                            i = 10;
                        ...
                        if (serverAcceptData.b > i) {
                            if (!serverAcceptData.c) {
                                ...
                                GameEngine.m2e("DOS: Too many attempts:" + serverAcceptData.b + " ip:" + inetAddress.toString());
                            }
                            if (serverAcceptData.b > 300 && !serverAcceptData.d) {
                                serverAcceptData.d = true;
                                GameEngine.m2e("DOS: Excessive attempts:" + serverAcceptData.b + " ip:" + inetAddress.toString());
                            }
                            return false;
                        }
                        z2 = true;
                ...
                if (!z2) {
                    ...
                    serverAcceptData4.f570a = inetAddress;
                    this.k.add(serverAcceptData4);
                }
                if (this.m > 500) {
                    if (!this.p) {
                        this.p = true;
                        GameEngine.m2e("DOS: Too many unique attempts: " + this.m + ". udp:" + this.f);
                        return false;
                    }
                    return false;
                }
                ...
                if (i2 > i4) {
                    if (!this.q) {
                        this.q = true;
                        GameEngine.m2e("DOS: Too open connections from same ip:" + inetAddress.toString() + " (count:" + i2 + ") max:" + i4);
                        return false;
                    }
                    return false;
                } else if (i3 > 300) {
                    if (!this.o) {
                        this.o = true;
                        GameEngine.m2e("DOS: Too open connections locking down:" + inetAddress.toString() + " (count:" + i3 + ")");
                        return false;
                    }
                    return false;
                } else {
    ...
    public void startServerSocket(boolean udp) {
        ...
        RUDPServerSocket rUDPServerSocket = new RUDPServerSocket(this.netEngine.port, 0, null, true);
        rUDPServerSocket.a(new AnonymousClass1());
        this.serverSocket = rUDPServerSocket;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: com.corrodinggames.rts.gameFramework.j.ao$1 */
    /* loaded from: game-lib.jar:com/corrodinggames/rts/gameFramework/j/ao$1.class */
    public class AnonymousClass1 extends c {
        ...
        public boolean a(SocketAddress socketAddress) {
            ...
            GameEngine.m2e("AcceptFilter: Unhandled SocketAddress type:" + socketAddress.getClass().getName());
            return true;
        }
    }
            ...
            try {
                ...
                try {
                    ...
                    if (!a(inetAddress, true)) {
                        ...
                        String str = "Accepted new connection id:" + playerConnect.c + ".. (ip:" + ip + ")";
                        if (this.f) {
                            str = str + " (udp)";
                        }
                    ...
                    GameEngine.m2e("Got IOException on new player connection");
                    e.printStackTrace();
                }
                ...
                if (this.c) {
                    ...
                    i++;
                    GameEngine.m2e("ServerSocket-accept(" + (this.f ? "udp" : "tcp") + ") failed: " + e2.getMessage() + " (closed:" + this.serverSocket.isClosed() + ")");
                    if (i > 100) {
                        GameEngine.m2e("Too many server socket fails");
                        b();
                        return;
                    }
                    try {
                        a();
                        if (i2 < 3 && gameEngine.bX.D() > 0) {
                    ...
                    GameEngine.m2e("ServerSocket-accept(" + (this.f ? "udp" : "tcp") + "): Got expected IOException after closed socket");
                    return;
                }
            }
        }
