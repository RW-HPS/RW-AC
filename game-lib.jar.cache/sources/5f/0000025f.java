...
public final class m extends a {
    ...
    public void b(com.corrodinggames.rts.game.units.custom.j jVar, float f) {
        ...
        if (lVar.aB) {
            ...
            for (int i = 0; i < mVar.a; i++) {
                n nVar = (n) a2[i];
                if (nVar.D && jVar.B.a > 0 && a(jVar, nVar) == null) {
            ...
            if (nVar.w != null) {
                ...
                if (mVar2.size() == 0) {
                    ...
                    if (!(amVar instanceof y)) {
                        GameEngine.print("onCreateSpawnUnitOf: Warning " + amVar.mo1r().i() + " not an orderable unit type, cannot attach");
                        amVar.ci();
                    } else {
                        y yVar = (y) amVar;
                        if (jVar.a(yVar, nVar)) {
    ...
    public static boolean a(com.corrodinggames.rts.game.units.custom.j jVar, n nVar, y yVar) {
        com.corrodinggames.rts.game.units.custom.l lVar = jVar.x;
        short s = nVar.a;
        if (lVar.aA.a <= s && yVar != null) {
            GameEngine.print("setAttachedUnitLookup: slot:" + ((int) s) + " larger than max slot size:" + lVar.aA.a);
            return false;
        }
        if (jVar.C == null) {
